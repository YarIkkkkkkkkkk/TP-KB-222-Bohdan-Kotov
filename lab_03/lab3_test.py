...
from lab3 import get_data, save_data, addNewElement, deleteElement, updateElement, printAllList, Student
class TestLab2Script(unittest.TestCase): 
    def setUp(self): 
        self.test_file = "test.csv" 
        self.test_data = [ Student("Alice","9876543210","woman","Ukraine"), 
                          Student("Jhon","1234567890","man","USA") ]
        
    def test_get_data(self):
        data = get_data(self.test_file) 
        self.assertEqual(data[0].name, self.test_data[0].name)
        self.assertEqual(data[1].phone, self.test_data[1].phone)
    def test_save_data(self): 
        save_data(self.test_file, self.test_data) 
        loaded_data = get_data(self.test_file) 
        self.assertEqual(loaded_data[0].name, self.test_data[0].name)
        self.assertEqual(loaded_data[1].country, self.test_data[1].country)
    ...
    def test_updateElement(self): 
        with patch('builtins.input', side_effect=["Jhon", "Bob", "5555555555", "man", "Canada"]): 
